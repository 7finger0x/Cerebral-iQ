...
export default function VisualPatterns({ onFinish }) {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchItemsForDomain('gv_visual_patterns').then(data => {
      setItems(data);
      setLoading(false);
    });
  }, []);
    ...
    >
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <span style={{ fontSize: '0.9rem', color: 'var(--color-primary)', textTransform: 'uppercase' }}>
          Item {currentIndex + 1} of {items.length} (Visual-Spatial Gv)
        </span>
        <h2 style={{ fontSize: '1.5rem', marginTop: '1rem', color: 'var(--color-text-muted)' }}>
          {currentItem.stem}
        </h2>
        <div style={{ fontSize: '4rem', marginTop: '1rem', background: 'rgba(255,255,255,0.05)', padding: '2rem', borderRadius: '12px' }}>
          {currentItem.target}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        {currentItem.options.map((opt, idx) => (
          <button 
            ...
            style={{ padding: '1.5rem', fontSize: '2rem', borderRadius: '12px' }}
