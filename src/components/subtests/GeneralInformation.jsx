...
export default function GeneralInformation({ onFinish }) {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchItemsForDomain('gc_information').then(data => {
      setItems(data);
      setLoading(false);
    });
  }, []);
  ...
  if (loading || !currentItem) return <div style={{textAlign: 'center', padding: '2rem'}}>Initializing Gc...</div>;

  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <span style={{ fontSize: '0.9rem', color: 'var(--color-primary)', textTransform: 'uppercase' }}>General Knowledge (Gc)</span>
