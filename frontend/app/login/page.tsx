...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-6 relative overflow-hidden text-slate-200">
      {/* Background Decor */}
      ...
      >
        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-primary/20 rounded-2xl mx-auto flex items-center justify-center mb-6 shadow-2xl shadow-primary/30">
            <Brain className="text-primary w-8 h-8" />
          </div>
          <h1 className="text-3xl font-bold font-orbitron tracking-tight mb-2 uppercase">
            {mode === 'login' ? 'Internal Access' : 'Create Node'}
          </h1>
          <p className="text-slate-400 text-sm">
            {mode === 'login' ? 'Cerebral iQ Research & Assessment Node' : 'Initialize Clinical Profile'}
          </p>
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 pl-12 pr-6 text-sm focus:border-primary/50 focus:bg-white/10 outline-none transition-all placeholder:text-slate-700 font-medium"
                required
              />
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            {error && (
              <motion.div 
                ...
                exit={{ opacity: 0 }}
                className={`${error.includes('successful') ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-red-500/10 border-red-500/20 text-red-500'} text-xs p-4 rounded-xl flex items-start gap-4 border`}
              >
                <Shield className="w-4 h-4 shrink-0" />
                <span>{error.includes('Verification Failed') ? error : (mode === 'login' ? `Verification Failed: ${error}` : error)}</span>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="space-y-4">
            ...
            >
              {loading ? (
                ...
                <>
                  {mode === 'login' ? 'Authorize Entry' : 'Register Profile'}
            ...
            <button
              ...
              className="w-full text-center text-xs text-slate-500 hover:text-white transition-colors py-2 font-medium tracking-wide"
            >
              {mode === 'login' ? "Don't have an endpoint? Create Account" : "Back to Authorization"}
            </button>
          </div>
        </form>
